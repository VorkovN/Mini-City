...
bool Help::execute()  const
{
	...
	std::cout << "buy_passenger_train - to buy new passenger train (_city name, car type, cars count)" << std::endl;
	std::cout << "buy_freight_train - to buy new freight train (_city name, car type, cars count)" << std::endl;
	std::cout << "send_train - to send train to same _city" << std::endl;
